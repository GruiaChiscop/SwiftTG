...
public func decompressTgs(_ data: Data, maximumOutputSize: Int = 8 * 1024 * 1024) throws -> Data {
    guard !data.isEmpty else { throw TgsDecompressError.emptyInput }
    guard maximumOutputSize > 0 else { throw TgsDecompressError.inflateFailed(Z_MEM_ERROR) }
    ...
    try data.withUnsafeBytes { (inPtr: UnsafeRawBufferPointer) in
        ...
        repeat {
            try buffer.withUnsafeMutableBufferPointer { outBuf in
                ...
                if produced > 0 {
                    guard output.count <= maximumOutputSize - produced else {
                        throw TgsDecompressError.inflateFailed(Z_MEM_ERROR)
                    }
