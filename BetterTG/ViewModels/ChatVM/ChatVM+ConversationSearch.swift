...
extension ChatVM {
    ...
    private func loadConversationSearchPage(
        ...
        if conversationSearchUsesSecretMessages || isSecretChat {
        ...
        guard !Task.isCancelled, generation == conversationSearchGeneration else { return }
    ...
    private func selectConversationSearchResult(at index: Int) {
        guard conversationSearchResultIds.indices.contains(index) else { return }
        conversationSearchSelectedIndex = index
        navigateToMessage(id: conversationSearchResultIds[index])
    }
